...
!-------------------------------------------------------------------------------
   subroutine updateZ_new(z,dz,dzdt,dzdtLast,time,omega_ma,d_omega_ma_dtLast, &
      ...
      complex(cp), intent(in)    :: dzdt(n_mlo_loc,n_r_max)     ! Time derivative of z
      complex(cp), intent(inout) :: dzdtLast(n_mlo_loc,n_r_max) ! Time derivative of z of previous step
      real(cp),    intent(inout) :: d_omega_ma_dtLast         ! Time derivative of OC rotation of previous step
      real(cp),    intent(inout) :: d_omega_ic_dtLast         ! Time derivative of IC rotation of previous step
      real(cp),    intent(in) :: lorentz_torque_ma            ! Lorentz torque (for OC rotation)
      real(cp),    intent(in) :: lorentz_torque_maLast        ! Lorentz torque (for OC rotation) of previous step
      ...
      !-- local variables:
      real(cp) :: ddzASL_loc_new(n_lo_loc,n_r_max) ! only used if l_TO is true
      real(cp) :: w2                  ! weight of second time step
      real(cp) :: O_dt
      real(cp) :: d_omega_ic_dt,d_omega_ma_dt
      complex(cp) :: rhs(n_r_max)   ! RHS of matrix multiplication
      ...
      integer :: i, l, m, lm, lj, mi, l1m0,l1m1, nRHS, nR
      integer :: n_r_bot, n_r_top
      logical :: l10
      ...
      if ( lRmsNext ) workB_new(1:n_mlo_loc,1:n_r_max)=z(1:n_mlo_loc,1:n_r_max)
      ...
      if (l10 .and. l_z10mat) then         
         lm = map_glbl_st%lm2(1,0)
         if (.not. lZ10mat_new) then
#ifdef WITH_PRECOND_Z10
         ...
         if ( l_SRMA ) then
            ...
            d_omega_ma_dt=LFfac*c_lorentz_ma*lorentz_torque_ma
            rhs(1)=O_dt*c_dt_z10_ma*z(l1m0,1) + &
            &      w1*d_omega_ma_dt + w2*d_omega_ma_dtLast
         else
            rhs(1)=0.0_cp
         end if
         ...
         if ( l_SRIC ) then
            ...
            d_omega_ic_dt = LFfac*c_lorentz_ic*lorentz_torque_ic
            rhs(n_r_max)=O_dt*c_dt_z10_ic*z(l1m0,n_r_max) + &
            &            w1*d_omega_ic_dt + w2*d_omega_ic_dtLast
         else
            rhs(n_r_max)=0.0_cp
         end if
         ...
         do nR=2,n_r_max-1
            rhs(nR)=O_dt*dLh(lm)* &
            &       or2(nR)*z(l1m0,nR)+ w1*dzdt(l1m0,nR)+        &
            &       w2*dzdtLast(l1m0,nR)
         end do

#ifdef WITH_PRECOND_Z10
         do nR=1,n_r_max
            rhs(nR) = z10Mat_fac_new(nR)*rhs(nR)
         end do
#endif
         call solve_mat(z10Mat_new,n_r_max,n_r_max,z10Pivot_new,rhs)
         z(l1m0,1:rscheme_oc%n_max)=real(rhs(1:rscheme_oc%n_max))
      end if
      ...
      do lj=1,n_lo_loc
         ...
         do mi=1,nRHS
            ...
            if ( l_z10mat .and. (i == l1m0)) then
               ! TODO: the rhs for l=1 m=0 was computed previously!
               ! This should be skipped when building the rhs
               rhs1_new(n_r_max,mi)=1.0_cp 
               ...
               do nR=2,n_r_max-1
                  rhs1_new(nR,mi)=O_dt*dLh(lm)*or2(nR)*z(i,nR) + &
                  &                     w1*dzdt(i,nR) +               &
                  &                     w2*dzdtLast(i,nR)
         ...
         do mi=1,nRHS
            ...
            if ( m > 0 ) then
               z(i+mi,1:rscheme_oc%n_max)=rhs1_new(1:rscheme_oc%n_max,mi)
            else
               z(i+mi,1:rscheme_oc%n_max)= cmplx(real(rhs1_new(1:rscheme_oc%n_max,mi)), 0.0_cp,kind=cp)
            end if
         end do
      end do
    
       !-- set cheb modes > rscheme_oc%n_max to zero (dealiazing)
      z(:,rscheme_oc%n_max+1:n_r_max)=zero

      !-- Get derivatives:
      call get_ddr(z, dz, work_LMdist, n_mlo_loc, 1,     &
            &       n_mlo_loc,n_r_max, rscheme_oc, l_dct_in=.false.)
      call rscheme_oc%costf1(z,n_mlo_loc,1,n_mlo_loc)
      ...
      !    vanish and the angular moment about the (planetary) rotation axis
      !    is kept constant.
      if ( (l_correct_AMz .or. l_correct_AMe) .and. &
         &    ((l1m0 > 0) .or. (l1m1 > 0)) ) then
    
         call get_angular_moment_dist(z,omega_ic,omega_ma,          &
              &                  angular_moment_oc,angular_moment_ic,&
              &                  angular_moment_ma)
         angular_moment = angular_moment_oc + angular_moment_ic + &
         &                    angular_moment_ma
      end if
...
!       
      ...
      do nR=n_r_top,n_r_bot
         do i=1,n_mlo_loc
            ...
            &      ( work_LMdist(i,nR)   +(dLvisc(nR)-beta(nR)) *dz(i,nR) &
            &        -( dLvisc(nR)*beta(nR)+two*dLvisc(nR)*or1(nR)           &
            &        + dLh(lm)*or2(nR) + dbeta(nR)+ two*beta(nR)*or1(nR) ) * z(i,nR) )
    
            dzdtLast(i,nR)=dzdt(i,nR)-coex*Dif_new(i)
            if ( lRmsNext ) then
               dtV_new(i)= O_dt*dLh(lm)*or2(nR)*(z(i,nR)-workB_new(i,nR))
            end if
         end do
         if ( lRmsNext ) then
...
!          end do
      ...
      if ( l10 .and. l_z10mat ) then ! z10 term !
         ...
         if ( .not. l_SRMA .and. ktopv == 2 .and. l_rot_ma ) then
            d_omega_ma_dtLast=d_omega_ma_dt -            &
            &    coex * ( two*or1(1)*real(z(l1m0,1))-real(dz(l1m0,1)) )
         end if
         if ( .not. l_SRIC .and. kbotv == 2 .and. l_rot_ic ) THEn
            d_omega_ic_dtLast=d_omega_ic_dt +                    &
            &    coex * ( two*or1(n_r_max)*real(z(l1m0,n_r_max))- &
